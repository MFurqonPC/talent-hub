    ...
    <div class="py-8" x-data="{ showModal: false, editing: null, editForm: {} }">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex justify-between items-center">
                <p class="text-gray-600 text-sm">Isi link (GitHub/Behance/dll) atau unggah file sebagai bukti portfolio.</p>
                <button @click="showModal = true" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow">+ Tambah Portfolio</button>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($portfolios as $pf)
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    @php
                                        $badge = ['pending' => 'bg-yellow-100 text-yellow-700', 'approved' => 'bg-green-100 text-green-700', 'rejected' => 'bg-red-100 text-red-700'][$pf->status];
                                    @endphp
                                    <span class="px-2 py-1 rounded-full text-xs font-medium {{ $badge }}">{{ ucfirst($pf->status) }}</span>
                                </td>
                        ...
                        @empty
                            <tr><td colspan="6" class="px-4 py-6 text-center text-gray-400">Belum ada portfolio yang diajukan.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            <div @click.away="showModal = false" class="bg-white rounded-xl shadow-lg w-full max-w-md p-6">
                ...
                <form x-show="!editing" action="{{ route('mahasiswa.portfolios.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" required class="w-full rounded-lg border-gray-300 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Contoh: Landing Page UMKM">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="url" name="link" class="w-full rounded-lg border-gray-300 focus:ring-indigo-500 focus:border-indigo-500" placeholder="https://...">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Atau Unggah File</label>
                        <input type="file" name="file_path" accept=".pdf,.jpg,.jpeg,.png" class="w-full text-sm text-gray-600">
                        <p class="text-xs text-gray-400 mt-1">Isi salah satu: Link atau File. Maks 2MB.</p>
                    </div>
